...
public class ProjectEndpoint {
    ...
    @Operation(summary = "Добавить новых участников в проект")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "User created",
                    content = {@Content(mediaType = "application/json",
                            schema = @Schema(implementation = UserResponse.class))})})
    @PostMapping("/{projectId}/addMembers")
    public void addNewMembersToProject(@AuthenticationPrincipal JwtUser currentUser,
                                                                  @PathVariable UUID projectId,
                                                                  @RequestBody List<UUID> userIds) {
        projectService.addNewMembers(projectId, userIds);
    }

    @Operation(summary = "Получить все доступные проекты для пользователя")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200",
                    content = {@Content(mediaType = "application/json",
                            schema = @Schema(implementation = ProjectResponse.class))})})
